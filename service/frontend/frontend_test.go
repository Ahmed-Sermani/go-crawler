...
import (
	...
	"github.com/Ahmed-Sermani/go-crawler/service/frontend/mocks"
...
func (s *FrontendTestSuite) TestSubmitLink(c *gc.C) {
	...
	req := httptest.NewRequest("POST", submitLinkEP, nil)
	req.Form = url.Values{}
...
func (s *FrontendTestSuite) TestSearch(c *gc.C) {
	...
	fe.templateExecutor = func(_ *template.Template, _ io.Writer, data map[string]interface{}) error {
	...
	req := httptest.NewRequest("GET", searchEP+"?q=KEYWORD", nil)
	res := httptest.NewRecorder()
	fe.router.ServeHTTP(res, req)
...
func (s *FrontendTestSuite) TestPaginatedSearchOnSecondPage(c *gc.C) {
	...
	req := httptest.NewRequest("GET", searchEP+"?q=KEYWORD&offset=2", nil)
	res := httptest.NewRecorder()
	fe.router.ServeHTTP(res, req)
...
func (s *FrontendTestSuite) TestPaginatedSearchOnThirdPage(c *gc.C) {
	...
	req := httptest.NewRequest("GET", searchEP+"?q=KEYWORD&offset=4", nil)
	res := httptest.NewRecorder()
	fe.router.ServeHTTP(res, req)
...
func (s *FrontendTestSuite) TestPaginatedSearchOnLastPage(c *gc.C) {
	...
	req := httptest.NewRequest("GET", searchEP+"?q=KEYWORD&offset=8", nil)
	res := httptest.NewRecorder()
	fe.router.ServeHTTP(res, req)
...
func (s *FrontendTestSuite) mockIterator(ctrl *gomock.Controller, numResults int) *mocks.MockIterator {
	...
	it.EXPECT().Document().DoAndReturn(func() *indexer.Document {

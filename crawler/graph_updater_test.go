...
import (
	...
	mocks "github.com/Ahmed-Sermani/go-crawler/crawler/mocks"
...
func (s *GraphUpdaterTestSuite) TestGraphUpdater(c *gc.C) {
	...
	// Expect the original link to be upserted with a new timestamp and
	...
	// Then expect two edges to be created from the origin link to the
	...
	// Finally expect a call to drop stale edges whose source is the origin link.
...
func (lm linkMatcher) Matches(x interface{}) bool {
	...
	return lm.id == link.ID &&
		lm.url == link.URL &&
		!link.RetreivedAt.Before(lm.notBefore)
}
